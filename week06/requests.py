...
sqls = "insert into comment(content,star) values (%s,%s)"
...
def parse(html):
    ...
    obj = html.xpath('//*[@id="comments"]')

    res = set()
    for x in obj:
        data = (x.xpath('./short')[0], x.xpath('/span[2]')[0], city)
        res.add(data)
    return res
...
def page_turn():
    # 美剧疑犯追踪第二季的短评
    urls = 'https://movie.douban.com/subject/10549251/comments?sort=new_score&status=P'
    data = get_url(urls)
