...
def parse(html, city):
    ...
    res = set()
    for x in obj:
        data = (x.xpath('@data-positionname')[0], x.xpath('@data-salary')[0], city)
        res.add(data)
    return res
...
def page_turn():
    ...
    for city in urls.keys():
        url = tuple(urls[city]+str(page) for page in range(1, 7))
        for page in url:
            print(page)
            data = get_url(page, city)
            res.update(data)
            sleep(5)
